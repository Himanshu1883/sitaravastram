...
function fallbackLineItem(order: {
  ...
  return {
    ...
    details: order.orderId ? `Order ${order.orderId}` : '',
    quantity: 1,
    unitPrice: amount,
    lineTotal: amount,
  };
...
export function normalizeOrderLineItems(order: {
  ...
  raw.forEach((item, index) => {
    ...
    parsed.push({
      ...
      lineTotal: row.unitPrice > 0 ? to2(row.unitPrice * row.quantity) : 0,
    });
  });

  const withTotals = parsed
    .map((line, index) => {
      if (line.lineTotal > 0) return { ...line, srNo: index + 1 };
      const share = to2(
        (Number(order.subtotal) || Number(order.total) || 0) / Math.max(parsed.length, 1),
      );
      return { ...line, srNo: index + 1, unitPrice: share, lineTotal: share };
    })
