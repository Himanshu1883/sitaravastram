...
type Fabric = { name: string; icon: string; description: string; color: string; image: string };

export default function HomepageFabricsPanel({ api }: { api: Api }) {
  ...
  useEffect(() => {
    if (api.data?.fabrics) {
      setItems(api.data.fabrics as Fabric[]);
      setDirty(false);
    }
  }, [api.data?.fabrics]);

  const save = async () => {
    await api.saveBlock('fabrics', items);
    setDirty(false);
    setToast('Fabrics saved');
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((item, i) => (
          <AdminCard key={i} padding="md" className="space-y-3">
            <input
              value={item.name}
              onChange={e => {
                const next = [...items];
