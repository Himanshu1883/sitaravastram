...
export default function CouponsTab({ data, api }: { data: AdminData; api: AdminApi }) {
  ...
  const toggleActive = async (id: string, active: boolean) => {
    await api.toggleCoupon(id, !active);
  };

  return (
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-sm shadow-card p-5">
        <h3 className="font-heading text-lg font-semibold text-navy-700 mb-4">Create Coupon</h3>
          ...
          <tbody>
            {data.coupons.map(c => (
              <tr key={c.id} className="border-t border-gray-50">
                ...
                <td className="px-4 py-3">₹{c.minOrder.toLocaleString('en-IN')}</td>
                <td className="px-4 py-3">{c.usedCount}/{c.usageLimit}</td>
                <td className="px-4 py-3">
                  <button onClick={() => toggleActive(c.id, c.active)} className={`text-xs px-2 py-0.5 rounded-full ${c.active ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500'}`}>{c.active ? 'Active' : 'Inactive'}</button>
                </td>
              </tr>
            ))}
          </tbody>
