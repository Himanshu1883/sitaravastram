...
export default function ReturnsTab({ data, api }: { data: AdminData; api: AdminApi }) {
  const updateReturn = async (id: string, status: ReturnRequest['status']) => {
    await api.updateReturnStatus(id, status);
  };

  return (
    <div className="p-6 space-y-4">
      <h2 className="font-heading text-xl font-semibold text-navy-700">Returns Management</h2>
          ...
          <div key={ret.id} className="bg-white rounded-sm shadow-card p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-gray-500">Order #{ret.orderId} · {ret.date}</p>
