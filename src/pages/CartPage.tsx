...
export default function CartPage() {
  ...
  const applyCouponHandler = () => {
    const result = validateCoupon(coupon, total);
    if (!result.valid || !result.coupon) {
      setCouponError(result.error || 'Invalid coupon');
      dispatch(removeCoupon());
      return;
    }
    const disc = calculateDiscount(result.coupon, total);
    dispatch(applyCoupon({ code: result.coupon.code, discount: disc, coupon: result.coupon }));
    setCouponError('');
  };
    ...
    return (
      <div className="min-h-screen bg-cream-100">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <p className="font-inter text-base text-gray-500 mb-8 max-w-md mx-auto">Looks like you haven't added anything yet. Discover our stunning collections.</p>
              ...
              <div key={`${item.product.id}-${item.size}-${item.color}`} className="bg-white rounded-sm shadow-card p-5 flex gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <Link to={`/product/${item.product.slug}`} className="font-inter text-sm font-medium text-navy-700 hover:text-rosegold-500 transition-colors line-clamp-2 leading-snug">{item.product.name}</Link>
                  ...
                  <div className="flex items-center justify-between mt-4 flex-wrap gap-3">
                    ...
                    <p className="font-playfair text-lg font-semibold text-navy-700">₹{(item.product.price * item.quantity).toLocaleString('en-IN')}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-sm shadow-card p-6 sticky top-28">
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input type="text" value={coupon} onChange={e => setCoupon(e.target.value.toUpperCase())} placeholder="SITARA10" className="w-full pl-9 pr-3 py-2.5 border border-rosegold-200 rounded-sm text-sm font-inter focus:outline-none focus:border-rosegold-500" />
                  </div>
                  <button onClick={applyCouponHandler} className="px-4 py-2.5 bg-navy-700 text-white text-sm font-inter font-medium rounded-sm hover:bg-rosegold-500 transition-colors">Apply</button>
                </div>
                {applied.appliedCode && <p className="text-xs font-inter text-emerald-600 mt-1.5">✓ {applied.appliedCode} applied — ₹{discount.toLocaleString('en-IN')} off</p>}
              ...
              <div className="space-y-3 border-t border-rosegold-100 pt-5">
                <div className="flex justify-between text-sm font-inter"><span className="text-gray-600">Subtotal</span><span className="font-medium text-navy-700">₹{total.toLocaleString('en-IN')}</span></div>
                {discount > 0 && <div className="flex justify-between text-sm font-inter"><span className="text-emerald-600">Coupon Discount</span><span className="font-medium text-emerald-600">−₹{discount.toLocaleString('en-IN')}</span></div>}
                <div className="flex justify-between text-sm font-inter"><span className="text-gray-600">Shipping</span><span className={`font-medium ${shipping === 0 ? 'text-emerald-600' : 'text-navy-700'}`}>{shipping === 0 ? 'FREE' : `₹${shipping}`}</span></div>
                <div className="flex justify-between font-playfair text-lg font-semibold text-navy-700 border-t border-rosegold-100 pt-3"><span>Total</span><span>₹{finalTotal.toLocaleString('en-IN')}</span></div>
              </div>
              <button onClick={handleCheckout} className="btn-primary w-full flex items-center justify-center gap-2 mt-6">Proceed to Checkout <ArrowRight size={16} /></button>
              <p className="text-center text-xs font-inter text-gray-500 mt-3">Secure checkout powered by Razorpay</p>
            </div>
          </div>
        </div>
      </div>
