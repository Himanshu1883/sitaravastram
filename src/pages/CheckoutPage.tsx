    ...
    return (
      <div className="min-h-screen bg-cream-100 flex items-center justify-center px-4">
        <div className="bg-white rounded-sm shadow-luxury-lg p-10 max-w-lg w-full text-center">
          ...
          <p className="font-inter text-sm text-gray-600 leading-relaxed mb-2">Your order <span className="font-semibold text-rosegold-500">#{orderId}</span> has been placed successfully.</p>
          <p className="font-inter text-sm text-gray-500 mb-8">A confirmation has been sent to +91 {auth.phone || phone}. Estimated delivery: 4-7 business days.</p>
  ...
  return (
    <div className="min-h-screen bg-cream-100">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {auth.isLoggedIn && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-sm px-4 py-3 mb-6 text-sm font-inter text-emerald-700">
                  ...
                  <div className="max-w-sm">
                    {!otpSent ? (
                      <>
                        ...
                        <button onClick={() => phone.length === 10 && setOtpSent(true)} disabled={phone.length !== 10} className="btn-primary w-full disabled:opacity-50">Send OTP</button>
                      ...
                      <>
                        ...
                        <button onClick={() => setOtpSent(false)} className="w-full text-sm font-inter text-rosegold-500 mt-3 hover:text-navy-700">Change Number</button>
                      </>
                    )}
                  </div>
                </div>
                    ...
                    {items.map(item => (
                      <div key={`${item.product.id}-${item.size}`} className="flex gap-4 py-4 border-b border-rosegold-100">
                        ...
                        <div className="flex-1">
                          <p className="font-inter text-sm font-medium text-navy-700">{item.product.name}</p>
                ...
                <div>
                  ...
                  <div className="space-y-3 mb-6">
                    {[
                      ...
                      { value: 'cod' as const, label: 'Cash on Delivery', desc: '₹49 handling charge applies' },
                    ].map(method => (
                  ...
                  <div className="flex items-center gap-2 text-xs font-inter text-gray-500 mb-6"><Shield size={14} className="text-rosegold-500" />Your payment information is secure and encrypted</div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-sm shadow-card p-5 sticky top-28">
              ...
              <div className="space-y-2 pt-2 text-sm font-inter">
                ...
                {discount > 0 && <div className="flex justify-between"><span className="text-emerald-600">Coupon ({applied.appliedCode})</span><span className="text-emerald-600">−₹{discount.toLocaleString('en-IN')}</span></div>}
                <div className="flex justify-between"><span className="text-gray-600">Shipping</span><span className={shipping === 0 ? 'text-emerald-600' : ''}>{shipping === 0 ? 'FREE' : `₹${shipping}`}</span></div>
                {codFee > 0 && <div className="flex justify-between"><span className="text-gray-600">COD Fee</span><span>₹{codFee}</span></div>}
                <div className="flex justify-between font-playfair text-base font-semibold text-navy-700 pt-2 border-t border-rosegold-100"><span>Total</span><span>₹{total.toLocaleString('en-IN')}</span></div>
              </div>
            </div>
          </div>
        </div>
